...
def simple_polygon(points):
    """Takes a list of points (each of type Vec) and returns a simple polygon 
    that passes though all points"""
    ...
    for i in range(len(points)):
        sorting.append((PointSortKey(points[i], anchor), i))
...
def graham_scan(points):
    """Takes a list three or more pint (each of type Vec) and returns the 
    convex hull of the points as a list of point of type Vec"""
    sorted_points = simple_polygon(points)
    stack = [sorted_points[0], sorted_points[1], sorted_points[2]]
    for i in range(3, len(sorted_points)):
        while not is_ccw(stack[-2], stack[-1], sorted_points[i]):
            stack.pop()
